...
import type { ArchivedJob, AutomationRule, EvidenceRecord, Finding, FindingStatus, FleetNode, OtaRelease, OtaRollout, Project, ReconNode, Workflow, WorkflowRun, WorkspaceData } from './types'

type View = 'jobs' | 'projects' | 'evidence' | 'map' | 'automations' | 'workflows' | 'scopes' | 'findings' | 'timeline' | 'fleet'
...
export default function WorkspaceViews({ view, workspace, nodes, projectId, onProject, onCreate, onInspect, onCreateAutomation, onUpdateAutomation, onDeleteAutomation, onCreateWorkflow, onRunWorkflow, onCancelWorkflow, onUpdateWorkflow, onDeleteWorkflow, onCreateScope, onImportFindings, onCorrelateFindings, onSetFindingStatus, onSetFindingSuppression, onCreateRelease, onCreateRollout, onAdvanceRollout, onRollbackRollout }:
  ...
  const [workflowName, setWorkflowName] = useState('Readiness check')
  ...
  return <>
    ...
    {view === 'workflows' && <WorkflowView workflows={workspace.workflows.filter((item) => !projectId || item.project_id === projectId)} runs={workspace.workflow_runs.filter((item) => !projectId || item.project_id === projectId)} projectId={projectId} name={workflowName} setName={setWorkflowName} error={workflowError} onCreate={async () => { setWorkflowError(''); try { await onCreateWorkflow({ name: workflowName, description: 'Checks node identity and attached-network readiness', steps: [{ id: 'system', capability: 'system.info', arguments: {}, depends_on: [], retries: 1 }, { id: 'connectivity', capability: 'net.connectivity.check', arguments: {}, depends_on: ['system'], retries: 1 }] }) } catch (error) { setWorkflowError(error instanceof Error ? error.message : 'Could not create workflow') } }} onRun={onRunWorkflow} onCancel={onCancelWorkflow} onRename={(id, newName) => onUpdateWorkflow(id, { name: newName })} onDelete={onDeleteWorkflow} />}
...
function WorkflowView({ workflows, runs, projectId, name, setName, error, onCreate, onRun, onCancel, onRename, onDelete }: { workflows: Workflow[]; runs: WorkflowRun[]; projectId: string; name: string; setName: (value: string) => void; error: string; onCreate: () => Promise<void>; onRun: (id: string) => Promise<void>; onCancel: (id: string) => Promise<void>; onRename: (id: string, newName: string) => Promise<void>; onDelete: (id: string) => Promise<void> }) {
  const [editingId, setEditingId] = useState('')
  const [draftName, setDraftName] = useState('')
  const [rowErrorId, setRowErrorId] = useState('')
  const [rowError, setRowError] = useState('')
  return <section className="catalog-layout workflow-layout">
    <div className="panel create-card"><span className="kicker">WORKFLOW TEMPLATE</span><h2>Readiness check</h2><p>Creates a safe two-step flow: collect system identity, then verify connectivity on a capable node.</p><label className="field"><span>WORKFLOW NAME</span><input value={name} onChange={(event) => setName(event.target.value)} /></label>{!projectId && <div className="inspection-error">Select a project before creating a workflow.</div>}{error && <div className="inspection-error">{error}</div>}<button className="primary-action" disabled={!projectId || !name.trim()} onClick={onCreate}>CREATE WORKFLOW</button></div>
    <RecordGrid empty="No workflows configured for this project.">{workflows.map((workflow) => { const related = runs.filter((run) => run.workflow_id === workflow.id).sort((a, b) => b.updated_at_ms - a.updated_at_ms); const latest = related[0]; const active = latest && (latest.status === 'queued' || latest.status === 'running'); const editing = editingId === workflow.id; return <article className="record-card automation-card" key={workflow.id}><span className={`record-icon ${active ? 'live' : ''}`}>⇢</span><div><span className="kicker">{workflow.steps.length} STEP WORKFLOW</span>{editing ? <input autoFocus value={draftName} onChange={(event) => setDraftName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Escape') setEditingId('') }} /> : <h3>{workflow.name}</h3>}<p>{workflow.steps.map((step) => step.capability).join(' → ')}</p><small>{latest ? `Latest run ${latest.status} · ${latest.steps.filter((step) => step.status === 'complete').length}/${latest.steps.length} complete · ${stamp(latest.updated_at_ms)}` : `Created ${stamp(workflow.created_at_ms)}`}</small>{latest?.steps.some((step) => step.error) && <small className="inspection-error">{latest.steps.find((step) => step.error)?.error}</small>}{rowErrorId === workflow.id && <small className="inspection-error">{rowError}</small>}</div><div className="rule-actions">{editing ? <>
      <button disabled={!draftName.trim()} onClick={async () => { setRowErrorId(''); try { await onRename(workflow.id, draftName.trim()); setEditingId('') } catch (err) { setRowErrorId(workflow.id); setRowError(err instanceof Error ? err.message : 'Rename failed') } }}>SAVE</button>
      ...
      <button disabled={Boolean(active)} onClick={() => { setEditingId(workflow.id); setDraftName(workflow.name); setRowErrorId('') }}>RENAME</button>
