    ...
    def advance_rollout(self, rollout_id: str) -> dict:
        ...
        for target in batch:
            ...
            if node is None or "fleet.ota.apply" not in node.get("capabilities", []):
                ...
                continue
            try:
                response = self.coordinator.invoke(target["device_id"], "fleet.ota.apply", {"release": release})
                result = response.get("payload", {}).get("result", {})
                target.update(status="verified" if result.get("artifact_sha256") == release["artifact_sha256"] else "failed",
                              error="" if result.get("artifact_sha256") == release["artifact_sha256"] else "verification mismatch")
    ...
    def rollback_rollout(self, rollout_id: str) -> dict:
        ...
        for target in rollout["targets"]:
            ...
            if node is None or "fleet.ota.apply" not in node.get("capabilities", []):
                ...
                continue
            try:
                response = self.coordinator.invoke(target["device_id"], "fleet.ota.apply",
